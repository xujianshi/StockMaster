...
namespace XjsStock.Dao
{
    public abstract class BaseDao<T> where T : BaseBean, new()
    {
        ...
        {
            get { return " SELECT TOP 1  * FROM " + TableName + " WITH (NOLOCK) "; }
        }
        ...
        {
            get { return " SELECT TOP 10  * FROM " + TableName + " WITH (NOLOCK) "; }
        }

        /// <summary>
        /// 获取前100行
        /// </summary>
        public string SelectTop100
        {
            get { return " SELECT * FROM " + TableName + " WITH (NOLOCK) "; }
        }
        ...
        protected string TryToString(DataRow row, string columnName)
        {
            ...
            {
                return SlConvert.TryToString(row[columnName]);
            }
            return string.Empty;
        }
        ...
        protected int TryToInt32(DataRow row, string columnName, Int32 defaultValue = 0)
        {
            if (row.Table.Columns.Contains(columnName))
            {
                return SlConvert.TryToInt32(row[columnName], defaultValue);
            }
            return 0;
        }
        ...
        protected long TryToInt64(DataRow row, string columnName, Int64 defaultValue = 0)
        {
            if (row.Table.Columns.Contains(columnName))
            {
                return SlConvert.TryToInt64(row[columnName], defaultValue);
            }
            return 0;
        }
        ...
        protected Decimal TryToDecimal(DataRow row, string columnName, Decimal defaultValue = 0)
        {
            if (row.Table.Columns.Contains(columnName))
            {
                return SlConvert.TryToDecimal(row[columnName], defaultValue);
            }
            return 0;
        }
        ...
        protected DateTime TryToDateTime(DataRow row, string columnName)
        {
            if (row.Table.Columns.Contains(columnName))
            {
                return SlConvert.TryToDateTime(row[columnName]);
            }
